...
int main() {
  ...
  const int START_ARRAY_SIZE = 500;  // The starting size of testArray
  const string BIG_O_GUESS =
      "O(n squared)";  // Your guess at the big-O of the timed code
  ...
  for (int cycle = 0; cycle < N_RUNS; cycle++) {
    ...
    // TIMED SECTION START
    // this is a bubble sort which is an O(n^2) algorithm
